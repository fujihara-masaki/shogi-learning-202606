...
test("Yokofudori 4e bishop bundled line reaches every reviewed leaf", async ({ page }) => {
  ...
  await openingLineByExactTitle(page, "横歩取り△4五角")
    .getByRole("link", { name: "学習する", exact: true })
    .click();

  const reachBranch = async () => {
    const reset = page.getByRole("button", { name: "最初に戻る", exact: true });
    if (await reset.isEnabled()) await reset.click();
    for (let i = 0; i < 20; i += 1) {
      await page.getByRole("button", { name: "本線を一手進む", exact: true }).click();
    }
  };
  await reachBranch();
  const history = page.locator(".move-history .move-usi");
  ...
  for (const [label, leaf, count] of cases) {
    const card = branches.locator(".branch-card").filter({ hasText: label });
    await expect(card).toHaveCount(1);
    await card.getByRole("button", { name: /この変化を見る/ }).click();
    ...
    await expect(history.last()).toHaveText(`(${leaf})`);
    await expect(page.getByTestId("opening-feedback")).toContainText("この定跡手順を完了しました");
    await reachBranch();
  }
  ...
  const disclosure = page.getByTestId("opening-variation-disclosure");
  await disclosure.locator("summary").click();
  await expect(disclosure.locator(".opening-variation-jump")).toHaveCount(35);
  const common = Array(20).fill("1").join("-");
  await disclosure.getByRole("button", {
    name: `21手目 3d2d、USI 3d2d、▲2四飛の主流変化、経路 ${common}-2、ここへ移動`,
    exact: true,
  }).click();
  await page.getByRole("button", { name: "ここから本線を最後まで再生", exact: true }).click();
  await expect(history.last()).toHaveText("(8h1a+)");
  await disclosure.getByRole("button", {
    name: `この分岐点の本線へ切り替える、第21手の分岐点 ルート、経路 ${common}`,
    exact: true,
  }).click();
  await expect(history.last()).toHaveText("(B*7g)");

  const source = page.getByTestId("opening-source");
  await expect(source).toContainText("セクション: 概要");
  await expect(source.getByRole("link")).toHaveAttribute("href", /oldid=88731470/);
  await page.setViewportSize({ width: 360, height: 800 });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth)).toBe(true);
});
...
test("Yokofudori 3c bishop bundled line replays the complete cited sequence", async ({ page }) => {
  await page.goto("/openings");
  await showOpeningTypeLines(page, "横歩取り△3三角");
  const card = openingLineByExactTitle(page, "横歩取り△3三角");
  await expect(card).toHaveCount(1);
  await expect(card).toContainText("16手");
  await card.getByRole("link", { name: "学習する", exact: true }).click();
  ...
  await expect(history.last()).toHaveText("(2b3c)");
  await expect(next).toBeDisabled();
  await expect(page.getByTestId("opening-feedback")).toContainText("この定跡手順を完了しました");
  const board = page.getByTestId("shogi-board");
  await expect(board.locator('[data-square="33"]')).toHaveAccessibleName(/3三.*後手の角/);
  await expect(board.locator('[data-square="22"]')).toHaveAccessibleName(/2二.*空きマス/);
  await expect(board.locator('[data-square="21"]')).toHaveAccessibleName(/2一.*後手の桂/);

  await page.getByRole("button", { name: "一手戻る", exact: true }).click();
  await expect(history).toHaveCount(15); await expect(history.last()).toHaveText("(2d3d)");
  await next.click(); await expect(history.last()).toHaveText("(2b3c)");
  await page.getByRole("button", { name: "最初に戻る", exact: true }).click();
  await expect(history).toHaveCount(0);
  ...
  const source = page.getByTestId("opening-source");
  await expect(source).toContainText("セクション: 概要（冒頭の基本16手）");
  await expect(source).toContainText("引用対象の概要冒頭の基本16手をすべて収録");
  await expect(source.getByRole("link")).toHaveAttribute("href", /oldid=110423793/);
  await page.setViewportSize({ width: 360, height: 800 });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth)).toBe(true);
});
