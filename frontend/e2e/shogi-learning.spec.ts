...
test("opening page shows opening categories and representative types", async ({ page }) => {
  ...
  await page.getByTestId("opening-type-card").filter({ hasText: "中飛車" }).getByRole("button", { name: "関連定跡を見る" }).click();
  await expect(page.getByTestId("opening-related-lines")).toContainText("中飛車 の関連定跡ライン");
  await expect(page.getByTestId("opening-related-line-card")).toContainText("中飛車");
