...
const path = fileURLToPath(new URL("../../../backend/app/wikipedia_opening_artifacts/yokofudori-3c-bishop.json", import.meta.url));
...
function fixture() {
  ...
  return openingFromImportedLine({ id: 203, name: record.line_name, opening_type: "相居飛車", initial_sfen: record.initial_sfen,
    ...
    source: { name: "Wikipedia 横歩取り3三角", license_name: "CC BY-SA 4.0", license_url: "" },
  });
}

describe("Yokofudori 3c bishop production canonical line", () => {
  it("validates all 16 edges and canonical SFENs with tsshogi", () => {
    expect(record.nodes.map((node) => node.usi)).toEqual("7g7f 3c3d 2g2f 8c8d 2f2e 8d8e 6i7h 4a3b 2e2d 2c2d 2h2d 8e8f 8g8f 8b8f 2d3d 2b3c".split(" "));
  ...
  it("builds, replays, jumps, steps back, and stops at the linear leaf", () => {
    ...
    expect(expectedOpeningMove(opening, Array(15).fill(0))?.usi).toBe("2b3c");
    expect(expectedOpeningMove(opening, Array(16).fill(0))).toBeNull();
    const leaf = applyOpeningPath(opening, Array(16).fill(0));
    expect(leaf.moves).toHaveLength(16); expect(leaf.moves.at(-1)?.usi).toBe("2b3c");
    const back = applyOpeningPath(opening, Array(15).fill(0));
    expect(back.moves.at(-1)?.usi).toBe("2d3d"); expect(expectedOpeningMove(opening, Array(15).fill(0))?.usi).toBe("2b3c");
