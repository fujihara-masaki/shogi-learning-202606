...
export default function EditorBoard({ position, onChange }: Props) {
  ...
  return <div className="editor-board-layout" data-testid="editor-board">
    ...
    <div><div className="file-coords">{files.map(f=><span key={f}>{f}</span>)}</div><div className="board-row-container"><div className="board-grid editor-grid">{position.board.map((row,r)=>row.map((piece,f)=><button type="button" key={`${r}-${f}`} className={`board-cell ${selected?.[0]===r&&selected?.[1]===f?"selected":""}`} data-testid={`editor-square-${9-f}-${r+1}`} onClick={()=>click(r,f)}>{piece && <span className={`piece-face ${piece.color==="w"?"piece-white":""} ${PIECE_LABEL[piece.code].length>1?"piece-narrow":""}`}>{pieceText(piece)}</span>}</button>))}</div><div className="rank-coords">{RANKS.map(x=><span key={x}>{x}</span>)}</div></div></div>
