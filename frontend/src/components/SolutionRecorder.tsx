          ...
          <div className="board-row-container">
            <div className="board-grid editor-grid">
              {position.board.map((row, r) => row.map((piece, c) => (
                <button type="button" key={`${r}-${c}`} className={`board-cell ${selection?.kind === "square" && selection.row === r && selection.col === c ? "selected" : ""}`} data-testid={`recorder-square-${9 - c}-${r + 1}`} onClick={() => clickSquare(r, c)}>
