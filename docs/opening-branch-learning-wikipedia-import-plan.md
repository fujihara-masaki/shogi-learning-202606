...
1. **PR-E1 石田流・早石田系**（巨大化を避け、E1a〜E1cへ分割）
   ...
   - [x] **PR-E1c: 早石田戦法の多分岐** — `backend/app/wikipedia_opening_artifacts/haya-ishida.json`。Wikipedia「石田流」固定版 revision `107928861` の「早石田」節を provenance B / `diagram_reconstruction` として収録。main終端は7手目 `7h7f`、分岐点1、branch 3、node総数11、最大深さ7。△8四歩・△4二玉・△6二銀の各経路は局面図と周辺説明から再構成し、各leaf以後は未収録。D1b/D1c、backend python-shogi、frontend tsshogiで全edgeの合法性・SFEN一致を確認済み。
2. **PR-E2 横歩取り系**
3. **PR-E3 角換わり系**
4. **PR-E4 棒銀系**
5. **PR-E5 ゴキゲン・対ゴキゲン**
