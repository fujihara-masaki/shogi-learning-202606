...
2. **PR-E2 横歩取り系**（article/section単位の小PRへ分割し、E2全体は継続中）
   ...
   - [x] **PR-E2c: 横歩取り△4五角の基本手順・初期分岐** — `backend/app/wikipedia_opening_artifacts/yokofudori-4e-bishop.json`。前回は固定版をCodex端末から再取得できず保留したが、2026-09-08にChatGPTのWeb閲覧でWikipedia「横歩取り4五角」固定版 revision `88731470` の版表示と「概要」節を確認した外部出典レビューを採用した（Codex端末による原本取得ではない）。共通20手と21手目の▲7七角・▲2四飛・▲8七歩・▲3五飛を provenance A / `partial_explicit_sequence` として独立lineに収録し、35 node、semantic main 26手、最大深さ27手、1分岐点・4経路を検証した。本文で先に列挙される▲7七角を編集上の既定本線とし、26手目△8七銀後の▲7七馬 (`1a7g`) は `omitted_after`、各leafより深い変化、△2八歩への別応、若島・佐々木流、後段の歴史的手順、△3八歩・△4四角系統は未収録。
   - [x] **PR-E2d: 横歩取り△3三桂の基本16手** — `backend/app/wikipedia_opening_artifacts/yokofudori-3c-knight.json`。2026-09-12にChatGPTのWeb閲覧でWikipedia「横歩取り3三桂」固定版 revision `106803367` の導入部（「解説」節より前）を確認した外部出典レビューを採用した（Codex端末からの原本取得はHTTP 403で不可）。初手から16手目△3三桂 (`2a3c`) までを独立lineとして provenance A / `complete_for_cited_sequence` で収録した。このstatusは引用対象の連続16手をすべて収録したという限定的な意味で、戦法全体の網羅ではない。16 node、semantic main 16手、最大深さ16手、分岐点0、root-to-leaf 1経路をbackend python-shogi / frontend tsshogiで検証し、17手目の連続明示がないため `omitted_after=null` とした。△3三角・相横歩取り、「解説」節の歴史的記述・断片的手順、17手目以後、5二玉型、4一玉型、青野流、勇気流、およびPR-E2cの深い変化は未収録。
   - [x] **PR-E2e: 横歩取り△3三角の基本16手** — `backend/app/wikipedia_opening_artifacts/yokofudori-3c-bishop.json`。2026-09-21にChatGPTのWeb閲覧でWikipedia「横歩取り3三角」の通常URL本文と同ページ末尾の版識別を突合し、revision `110423793` の「概要」第1段落を確認した外部出典レビューを採用した。固定版URLの独立再取得はWebツールのCache miss等で成功せず、Codex端末からの取得もHTTP 403で不可だったため、固定版をCodexが直接取得したものではない。初手から16手目△3三角 (`2b3c`) までを平手初期局面からの独立direct-parent lineとして provenance A / `complete_for_cited_sequence` で収録した。このstatusは引用対象の基本16手をすべて収録したという限定的な意味で、戦法・記事全体の網羅ではない。16 node、semantic main 16手、最大深さ16手、分岐点0、root-to-leaf 1経路をbackend python-shogi / frontend tsshogiで検証した。`omitted_after=null` は今回の引用対象が16手で完結することを示し、記事に後続手順がないという意味ではない。概要後段の仮定手順、「歴史」「2一角の攻め」「竹部スペシャル」、17手目以後の▲3六飛、青野流、勇気流等は対象外とした。通常seed/catalog/bundled importへ組み込み、新規DB、PR-E2d相当DBへの初回追加、および新旧lineにruntime commentを設定した再実行で既存4 lineのID・stable key・USI/SFEN・direct parent・main/order/group・comment・source・分類関連が保持されることを専用backend testで確認した。最終検証はvalidator全bundled artifact、backend `398 passed`、frontend Vitest `220 passed`、lint、build、追加Playwright `1 passed`、Chromium全体 `98 passed`、`git diff --check` が成功した。
   - [ ] **PR-E2f以降** — 横歩取り△3三角の追加変化、横歩取り△8五飛／中座飛車、横歩取り△2三歩を、それぞれ根拠となるarticle/section単位で調査・レビューして扱う。5二玉型、4一玉型、青野流、勇気流等を根拠調査なしに完了対象へ加えず、E2全体は継続中とする。
3. **PR-E3 角換わり系**
4. **PR-E4 棒銀系**
5. **PR-E5 ゴキゲン・対ゴキゲン**
6. **PR-E6 鬼殺し系**
