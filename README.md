...
- 戦型別のランダム出題・未挑戦優先出題、100件を超える戦型の順次学習、最終問題の完了表示に対応しています。ただし「すべての戦型」から戦型ごとに均等な重みで選ぶランダム出題は未対応です。
- weak出題(苦手問題と復習の統合)、お気に入り、履歴画面統合、サーバー側sequential cursorは未対応です。一覧UI自体も先頭30件の表示のみで、本格的なページング・絞り込みは今後の課題です。
