...
詰め将棋・タイムアタック・復習・学習履歴に加え、固定データによる定跡学習MVPを実装しています。
...
E2Eテストは、ブラウザ上でホーム、詰め将棋プレイ、問題作成UI、盤面からの手順記録、作成問題のプレイ、編集・削除、定跡学習(手順学習・次の一手の両モード)、タイムアタックの基本表示を自動確認するためのテストです。次の一手モードのテスト(`e2e/next-move.spec.ts`)は、E2E用DBに定跡DBデータが無いため learning_samples API をモックして実行します。
...
| 定跡学習 | 「定跡手順を学ぶ」(定跡手順のなぞり学習)と「次の一手に挑戦する」(局面から一手を考える問題)の2モード |
...
定跡学習トップ(`/openings`)には2つの学習モードがあります。

- **定跡手順を学ぶ**: 定跡の手順を一手ずつ盤面でなぞって覚えるモード(従来の定跡学習)。
- **次の一手に挑戦する**: 定跡DBから抽出した局面を見て、自分で一手を考えるモード。
...
1. 定跡学習トップで「次の一手に挑戦する」を選びます(`/openings?mode=next-move`)。
2. 戦型を選び(初期状態では先頭の戦型が選択されます)、問題カードの「挑戦する」を押します(`/openings/next-move/:id`)。
...
5. 解答後は、候補順位・評価値・最上位候補との評価値差・進行例(PV)・出典/ライセンスを確認でき、「候補手を比較する」で全候補の一覧表を表示できます。「次の問題」「もう一度考える」「定跡一覧へ戻る」の導線があります。
...
#### 次の一手モードの既知の制限(今後の拡張候補)

- 出題は戦型単位です。API の並び順の都合で全戦型混在の取得は特定の戦型に偏るため、「すべての戦型」からの均等抽出・ランダム出題・ページングは未対応です(残課題)。
- 「次の問題」は同一戦型の先頭最大100件の中での巡回です。100件を超える問題や戦型をまたぐ出題ポリシーは未対応です(残課題)。
