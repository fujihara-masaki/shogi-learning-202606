...
通常DBは問題・定跡・学習履歴などを保存し、次の一手専用DBは外部定跡の候補手と学習サンプルを供給します。役割、既定パス、準備方法は[データ管理ガイドの「データとDBの区分」](docs/data-management.md#データとdbの区分)を参照してください。

### やねうら王定跡からの学習用サンプル抽出

次の一手で使う学習サンプルは同梱の標準件数ではなく、利用条件を確認した外部定跡から別途抽出します。取り込み、抽出、検証、安全な差し替えは[データ管理ガイドの「外部定跡と次の一手サンプル」](docs/data-management.md#外部定跡と次の一手サンプル)を参照してください。
