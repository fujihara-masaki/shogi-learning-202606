...
- アプリ本体のソースコードは GPL-3.0 ベースで扱う方針です。詳細は root の `LICENSE` を参照してください。
...
E2Eテストは、ブラウザ上でホーム、詰め将棋プレイ、問題作成UI、盤面からの手順記録、作成問題のプレイ、編集・削除、定跡学習、タイムアタックの基本表示を自動確認するためのテストです。
...
| 定跡学習 | 固定サンプル定跡の一覧、盤面上での期待手入力、ヒント、1手戻る、最初から |
