...
def test_import_opening_file_and_query_api(client, tmp_path):
    ...
    assert imported_line["opening_type"] == "中飛車"
    assert imported_line["opening_type_id"] is not None

    related = client.get(f"/opening-types/{imported_line['opening_type_id']}/lines")
    assert related.status_code == 200
    assert any(line["id"] == imported_line["id"] for line in related.json())

    detail = client.get(f"/api/openings/{imported_line['id']}")
    assert detail.status_code == 200
    data = detail.json()
    assert data["opening_type_id"] == imported_line["opening_type_id"]
...
def test_seed_opening_mvp_apis(client):
    ...
    nakabisha = next(line for line in lines.json() if line["name"] == "中飛車")
    assert nakabisha["opening_type_id"] is not None
    related = client.get(f"/opening-types/{nakabisha['opening_type_id']}/lines")
    assert related.status_code == 200
    assert [line["name"] for line in related.json()] == ["中飛車"]
...
def test_opening_catalog_seed_apis(client):
    ...
    nakabisha = next(opening_type for opening_type in type_body if opening_type["name_ja"] == "中飛車")
    assert nakabisha["opening_line_count"] == 1
    lines = client.get(f"/opening-types/{nakabisha['id']}/lines")
    assert lines.status_code == 200
    assert [line["name"] for line in lines.json()] == ["中飛車"]

    yagura = next(opening_type for opening_type in type_body if opening_type["name_ja"] == "矢倉")
    assert yagura["opening_line_count"] == 0
    assert client.get(f"/opening-types/{yagura['id']}/lines").json() == []
