...
SAMPLE_OPENING_LINES = [
    ...
    },
    {
        "name": "横歩取り△3三桂",
        "opening_type": "相居飛車",
        "opening_type_name": "横歩取り△3三桂",
        "description": "横歩取りの基本局面から後手が3三桂と跳ねる戦法です。",
        "tag": "yokofudori",
        "moves": ["7g7f", "3c3d", "2g2f", "8c8d", "2f2e", "8d8e", "6i7h", "4a3b", "2e2d", "2c2d", "2h2d", "8e8f", "8g8f", "8b8f", "2d3d", "2a3c"],
    },
    {
        "name": "横歩取り△3三角",
        "opening_type": "相居飛車",
        "opening_type_name": "横歩取り△3三角",
        "description": "横歩取りの基本局面から後手が2二の角を3三へ上がる戦法です。",
        "tag": "yokofudori",
        "moves": ["7g7f", "3c3d", "2g2f", "8c8d", "2f2e", "8d8e", "6i7h", "4a3b", "2e2d", "2c2d", "2h2d", "8e8f", "8g8f", "8b8f", "2d3d", "2b3c"],
    },
    {
        "name": "相横歩取り",
        "opening_type": "相居飛車",
        "opening_type_name": "相横歩取り",
        "description": "双方が横歩を取り、飛車交換を含む激しい展開へ進む相居飛車です。",
        "tag": "yokofudori",
        "moves": ["7g7f", "3c3d", "2g2f", "8c8d", "2f2e", "8d8e", "6i7h", "4a3b", "2e2d", "2c2d", "2h2d", "8e8f", "8g8f", "8b8f", "2d3d", "2b8h+", "7i8h", "8f7f", "8h7g", "7f7d", "3d7d"],
        "comments": ["角道を開けます。", "後手も角道を開けます。", "飛車先を伸ばします。", "後手も飛車先を伸ばします。", "2筋の歩交換を狙います。", "後手も8筋の歩交換を狙います。", "金を7八へ上がります。", "後手も金を3二へ上がります。", "2筋の歩を突き捨てます。", "後手は同歩と応じます。", "飛車で歩を取ります。", "後手も8筋を突き捨てます。", "先手は同歩と応じます。", "後手飛車も歩を取ります。", "3四の横歩を取ります。", "後手は角を交換します。", "銀で角を取り返します。", "後手飛車も7六の横歩を取ります。", "銀で7七を受ける主流形です。", "後手飛車は7四へ引きます。", "飛車を交換して超急戦へ進みます。"],
    },
    {
        "name": "石田流",
        "opening_type": "振り飛車",
...
OPENING_TYPE_SEEDS = [
    ...
    ("相居飛車", "横歩取り", "横歩取り△3三角", "よこふどりさんさんかく", "Side Pawn Picker 3c Bishop", ["横歩取り3三角", "横歩取り空中戦法"], "横歩取りの基本局面から後手が2二の角を3三へ上がる戦法です。", "Wikipedia 横歩取り3三角", "https://ja.wikipedia.org/w/index.php?title=%E6%A8%AA%E6%AD%A9%E5%8F%96%E3%82%8A3%E4%B8%89%E8%A7%92&oldid=110423793", "CC BY-SA", 44),
...
def upsert_opening_move_nodes(conn, line_id: int, move_nodes: list[dict]) -> dict[str, int]:
    ...
    for node in move_nodes:
        ...
        params = [line_id, node["usi"]]
        if parent_id is not None:
            params.append(parent_id)
        candidates = conn.execute(
            f"""SELECT id, comment FROM opening_line_moves
                WHERE line_id=? AND usi=?
                  AND {parent_clause} AND move_key = 'legacy-' || id ORDER BY id""",
            params,
        ).fetchall()
        available = [row for row in candidates if int(row["id"]) not in claimed_ids]
        if len(available) == 1:
...
BUNDLED_WIKIPEDIA_OPENING_ARTIFACTS = (
    ...
    "yokofudori-3c-knight.json",
    "yokofudori-3c-bishop.json",
)
